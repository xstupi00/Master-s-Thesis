...
\cite{cegar}
    A \emph{family of MCs} $\fml$ is a~quadruple $\family$  where $S$ is a~finite set of \emph{states}, $\sinit \in S$ is an~\emph{initial state}, $K$ is a~finite set of parameters with domains $V_k  \subseteq S$ for each $k \in K$, and $\fpm : S \rightarrow Distr(K)$ is a~family of transition probability functions.
\end{definition}

% Function $\fpm$ of a~family $\fml$ 
