...
Since the~loop takes at the~input a~pMDP $\mathcal{M}$ and wants to analyse it,~we need to construct a~parameter substitution $\mathcal{M}_{sub}^r$ of the~given MDP model $\mathcal{M}$ concerning the~parameter region $r$.
This parameter substitution yields a~stochastic game,~and by analysing it,~we can derive whether holds that $\mathcal{M}_{sub}^r \models \varphi$ for each $\varphi \in \varPhi$.

This model checking of the~constructed stochastic game yields the~refinable lower and upper bounds on probabilities correspond to the~current parameter space intervals.
...
% It selects from the~parameters set,~which includes both kinds of parameters,the~parameter with the~greatest number of possible options or parameter containing the`longest interval,~respectively.
% The~domain of the~selected parameter is splitting in half,~and the~analysed family is refined according to that.
% This naive strategy offers space for improvement in the~future,~and it is in our plans to improve it concerning the~model of the analyzed family.