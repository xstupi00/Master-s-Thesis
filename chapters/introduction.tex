...
These undefined system specifications are called \textit{holes},~and they can,~e.g.,~reflect an~unspecified component   
%for wireless specification 
or a~partially implemented controller.
...
A~\textit{one-by-one} approach can naively solve the~synthesis problem by analysing all unique designs~\cite{profeat,onebyone}.
...
\MC{Tady pozor a presneji. Mluvil bych o idnuktivni synteze a Roman nebyl prvni, prvni byl nas TACAS a FM paper} In this work,~we will focus on a~complete state-of-the-art approach for the~synthesis of probabilistic programs.
